...
// 오디오 객체
const audio = {
    glow: new Audio('Glow3.mp3'),
    error: new Audio('Error3.mp3')
};
...
function startGame() {
    ...
    // 음식 생성 시작
    createFood();
    gameState.foodInterval = setInterval(createFood, 2000);
...
function handleFoodClick(foodElement, food) {
    ...
    // 건강한 음식인 경우 yamy 이미지로 이동
    if (food.points > 0) {
        moveFoodToYamy(foodElement);
    } else {
        // 건강하지 않은 음식은 바로 제거
        foodElement.remove();
    }
...
// 음식을 yamy 이미지로 이동
function moveFoodToYamy(foodElement) {
    const yamyImage = document.querySelector('.yamy-image');
    if (!yamyImage) {
        console.log('yamy 이미지를 찾을 수 없습니다.');
        foodElement.remove();
        return;
    }
    
    // yamy 이미지의 위치 가져오기
    const yamyRect = yamyImage.getBoundingClientRect();
    const foodRect = foodElement.getBoundingClientRect();
    
    // 이동할 거리 계산 (yamy 이미지 중앙으로)
    const targetX = yamyRect.left + yamyRect.width / 2 - foodRect.width / 2;
    const targetY = yamyRect.top + yamyRect.height / 2 - foodRect.height / 2;
    
    console.log('음식 이동 시작:', {
        현재위치: { x: foodRect.left, y: foodRect.top },
        목표위치: { x: targetX, y: targetY },
        yamy위치: { x: yamyRect.left, y: yamyRect.top }
    });
    ...
    foodElement.style.transition = 'all 0.8s ease-in-out';
...
function showLevelUpMessage() {
    ...
    `;
    message.textContent = `레벨 ${gameState.level} 달성! 🎉`;
